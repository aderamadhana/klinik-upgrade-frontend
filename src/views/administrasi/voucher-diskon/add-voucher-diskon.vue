<template>
  <div>
    <div class="page-header">
      <div>
        <h1 class="page-title">Tambah Voucher Diskon</h1>
        <p class="page-subtitle">
          Buat voucher diskon baru dengan mode direct atau generate voucher
        </p>
      </div>
          ...
          <v-card elevation="1" class="mb-4">
            ...
            <v-card-text>
              ...
              <v-row>
                ...
                <v-col cols="12" md="8">
                  <v-alert type="info" variant="tonal" rounded="lg">
                    Item voucher bisa dikonfigurasi setelah voucher dibuat
                    melalui tombol konfigurasi pada halaman daftar voucher.
                  </v-alert>
                </v-col>
              </v-row>
            </v-card-text>
  ...
  methods: {
    async fetchMasterData() {
      ...
      try {
        ...
        this.setDefaultKategoriVoucher();
      } catch (error) {
        console.error(error);
    ...
    setDefaultKategoriVoucher() {
      if (
        this.form.kategori_voucher_id ||
        !this.kategoriVoucherOptions.length
      ) {
        return;
      }

      const directItem =
        this.kategoriVoucherOptions.find((item) => item.kode === "DIRECT") ||
        this.kategoriVoucherOptions.find((item) =>
          String(item.label || "")
            .toLowerCase()
            .includes("direct"),
        ) ||
        this.kategoriVoucherOptions[0];

      this.form.kategori_voucher_id = directItem.value;
      this.syncModeFromKategori(directItem.value);
    },
    ...
    async submitForm() {
      ...
      try {
        await voucherDiskonService.create(this.payload);

        this.showSnackbar("Voucher diskon berhasil disimpan", "success");
        ...
        this.showSnackbar(
          this.getErrorMessage(error, "Gagal menyimpan voucher diskon"),
          "error",
        );
      } finally {
        this.loading = false;
