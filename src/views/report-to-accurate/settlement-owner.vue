    ...
    >
      <div>
        <div class="text-h5 font-weight-bold text-high-emphasis">
          Settlement Accurate (Owner)
        </div>
        <div class="text-body-2 text-medium-emphasis mt-1">
          Upload faktur penjualan harian transaksi Owner ke Accurate berdasarkan
          invoice lunas.
        </div>
      </div>
      ...
      <v-card rounded="lg" max-height="88vh" class="overflow-hidden">
            ...
            <div class="d-flex align-center ga-3">
              ...
              <div>
                <div class="text-subtitle-1 font-weight-bold text-white">
                  Upload Faktur Owner ke Accurate
                </div>
                <div class="text-body-2 text-white">
                  Kirim data sebagai faktur penjualan harian Owner.
                </div>
              </div>
            </div>
        ...
        <v-card-text class="pa-4">
          <v-alert
            ...
            class="mb-4"
          >
            Data akan dikirim sebagai faktur penjualan harian Owner. Proses ini
            tidak boleh diulang jika Accurate sudah berhasil menerima faktur.
          </v-alert>
          ...
          <v-card class="rounded-lg border" elevation="0">
            <v-card-text class="pa-4">
              <div class="d-flex align-start ga-3 mb-4">
                ...
                <div>
                  ...
                  <div class="text-body-2 text-medium-emphasis">
                    Pastikan tanggal faktur, cabang, total system, dan deskripsi
                    sudah sesuai.
                  </div>
                </div>
              </div>
              ...
              <v-list density="compact" lines="two" border rounded>
                ...
                <v-list-item>
                  <v-list-item-title>Total System</v-list-item-title>
                  <v-list-item-subtitle>
                    <span class="font-weight-bold text-success">
                      {{ formatCurrency(selectedRow?.total_system || 0) }}
                    </span>
                  </v-list-item-subtitle>
                </v-list-item>
...
export default {
  ...
  methods: {
    ...
    async loadData() {
      ...
      this.loading = true;
      try {
        const response = await accurateSettlementService.getOwner({
        ...
        this.showSnackbar(
          this.getErrorMessage(
            error,
            "Gagal mengambil data settlement Accurate Owner.",
          ),
          "error",
        );
      } finally {
    ...
    async uploadSelected() {
      ...
      try {
        const response = await accurateSettlementService.uploadOwner({
          tanggal_faktur: this.selectedRow.tanggal_faktur,
          toko_id: this.selectedRow.toko_id,
        });

        this.showSnackbar(
          response?.message || "Faktur Owner berhasil diupload ke Accurate.",
          "success",
        );
        ...
        this.showSnackbar(
          this.getErrorMessage(error, "Upload faktur Accurate Owner gagal."),
          "error",
        );
      } finally {
