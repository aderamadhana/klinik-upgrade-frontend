<template>
  <div>
    <div class="page-header">
      <div>
        <h1 class="page-title">Tambah Counter Antrian</h1>
        <p class="page-subtitle">
          Input counter manual atau samakan counter dari cabang lain
        </p>
      </div>
    ...
    <v-card elevation="1">
      ...
      <v-card-text class="pt-6">
        ...
        >
          ...
          <template v-else>
            <div class="text-subtitle-1 font-weight-bold mb-3">
              Samakan Counter dengan Cabang Lain
            </div>
            ...
            <v-row>
              <v-col cols="12" md="5">
                <v-autocomplete
                  ...
                  label="Ambil Data Dari Cabang *"
                  placeholder="Pilih cabang sumber"
                  :items="tokoOptions"
                  item-title="label"
                  item-value="value"
                  variant="outlined"
          ...
          <div class="d-flex flex-column flex-md-row justify-end ga-3 mt-6">
            ...
            <v-btn
              ...
              :disabled="loadingSave"
            >
              {{ mode === "manual" ? "Simpan Counter" : "Samakan Cabang" }}
            </v-btn>
          </div>
        </v-form>
      </v-card-text>
...
export default {
  ...
  data() {
    return {
      ...
      breadcrumbs: [
        ...
        { title: "Tambah Counter Antrian", disabled: true },
      ],
  ...
  computed: {
    targetTokoOptions() {
      return this.tokoOptions.filter((item) => {
        return Number(item.value) !== Number(this.copyForm.source_toko_id);
      });
    },
  ...
  methods: {
    ...
    async submitManual() {
      ...
      try {
        await masterAntrianCounterService.create(this.payload);

        this.showSnackbar("Counter antrian berhasil disimpan", "success");
        ...
        this.showSnackbar(
          this.getErrorMessage(error, "Gagal menyimpan counter antrian"),
          "error",
        );
      } finally {
    ...
    async submitCopy() {
      ...
      try {
        ...
        this.showSnackbar(
          "Counter berhasil disamakan dengan cabang tujuan",
          "success",
        );
        ...
        this.showSnackbar(
          this.getErrorMessage(error, "Gagal menyamakan counter antrian"),
          "error",
        );
      } finally {
        this.loadingSave = false;
